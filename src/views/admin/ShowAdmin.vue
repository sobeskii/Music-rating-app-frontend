  ...
  <app-layout :adminPanelActive="true">
    ...
    <div class="max-w-7xl mx-auto flex m-2 px-3 justify-start my-2 flex-wrap md:gap-0 flex-row mt-2 md:mt-4">
                  ...
                  <ul class="space-y-2">
                    <li>
                      <router-link :to="{name : 'admin'}" class=
                        ...
                        <i class="lg:text-base text-xs flex-shrink-0 text-white transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white fa-solid fa-users"></i>
                        <span class="lg:text-base text-xs flex-1 ml-3 whitespace-nowrap">Users</span>
                      </router-link>
                    </li>
                    <li>
                      <router-link :to="{name : 'flagged'}" class="flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
                        <i class="lg:text-base text-xs flex-shrink-0 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white fa-solid fa-asterisk"></i>
      ...
      <div class="flex-none w-full min-w-min my-2 md:w-9/12 justify-center">
        <custom-table   :hasRecords="(this.users != null)  ? (users.data.length > 0) ? true : false : false"
                        :isLoading="(this.users == null)  ? true : false"
        ...
        >
          ...
          <template v-slot:head >
            ...
            <th
                class="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 hidden sm:table-cell uppercase md:w-1/5 w-1/4">
              Role
            </th>
            <th
                class="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase hidden lg:table-cell md:w-1/5 w-1/5 ">
              Created at
            </th>
            <th
          ...
          <template v-if="(this.users != null)  ? (users.total > params.perPage) ? true : false : false" v-slot:pagination>
...
export default {
  created() {
    ...
    this.params.page = 1;
    this.retrieveUserList(this.params);
  },
  data() {
    return {
      params: {
  ...
  methods:{
    search() {
      ...
      throttle(this.retrieveUserList(this.params), 200);
    },
    changePerPage() {
      this.retrieveUserList(this.params);
    },
    nextPage() {
      if(this.users.current_page != this.users.last_page){
        this.params.page = this.users.current_page + 1;
        this.retrieveUserList(this.params);
      }
    },
    previousPage() {
  ...
  computed:{
    parseEntryCount(){
      ...
      return "Showing " +
          parseInt( (this.params.perPage * (this.users.current_page - 1)  + 1) ) +" to " +
          entryLimit + " of "+ this.users.total;
    },
    ...mapState({
      users: state => state.admin.users.users,
      request_items: state => state.admin.users.request_items,
    }),
  }
}
</script>
