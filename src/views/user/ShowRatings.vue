  ...
  <app-layout>
    ...
    <div class="max-w-7xl mx-auto flex m-2 px-3 justify-start my-2 flex-wrap md:gap-8 flex-row mt-2 md:mt-4">
              ...
              <aside class="w-full ml-auto" aria-label="Sidebar">
                <div class="overflow-y-auto py-4 px-3 bg-white rounded dark:bg-gray-800">
                  <ul class="space-y-2">
                    ...
                    <li>
                      <router-link :to="{name:'user_ratings',params: { id: this.$route.params.id }}" class="flex items-center p-2
                              text-white bg-green-500
                              text-base font-normal rounded-lg">
                        <i class="lg:text-base text-xs flex-shrink-0 text-white transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white fa-solid fa-star"></i>
      ...
      <div class="flex-none w-full min-w-min my-2 md:w-9/12 justify-center">
        ...
        <review-thread :isLoading="ratings == null"
                       :loadMore="LoadMoreReviews"
                       :reviewsShowing="reviewsToShow"
                       :reviews="this.availableUserReviews"
                       :belongsToUserProfileReaction="true"
                       :hasLikes="false"
                       emptyMsg="User has not reviewed anything"
        ></review-thread>
      </div>
      </template>
      <template v-else>
...
export default {
  created() {
    this.getUserRating(this.$route.params.id);
  },
  components: {
    BulletListLoader,
    AppLayout,
  ...
  computed: {
    ...mapGetters(['availableUserReviews']),
    ...mapState({
      ratings: state => state.rating.user_ratings,
